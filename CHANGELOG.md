...
- README images and doc links now resolve outside npmjs.com. `docs/` and `e2e/` are not part of the published tarball, so the relative paths pointed nowhere for anything reading the package directly (editor README previews, registry mirrors, CDN file views); the npm package page happened to work only because it rewrites relative links against the repository's default branch. They are now absolute GitHub URLs. Docs only — the built output is identical to 0.3.1.
...
[0.3.2]: https://github.com/ohgree/dom-cutout/compare/v0.3.1...v0.3.2
